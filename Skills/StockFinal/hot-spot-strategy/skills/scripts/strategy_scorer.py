    ...
    def score(self, stock_data: Dict, market_data: Dict = None) -> Optional[StockScore]:
        ...
        Args:
            stock_data: {
                'code': str,
                'name': str,
                'retrace_pct': float,       # 回调幅度%（负值）
                'retrace_days': int,         # 回调天数
                'volume_shrink_pct': float,  # 缩量程度%（相对涨停日）
                'stop_signal': str,          # 止跌信号类型
        ...
        return StockScore(
            ...
            signal_level=signal_level,
            suggested_position_pct=position_pct,
            buy_timing="企稳信号当日尾盘(14:50后)或次日早盘",
            stop_loss_pct=-7.0,
            time_stop_days=7
        )
...
class NewStockScorer(StrategyScorer):
    ...
    def score(self, stock_data: Dict, market_data: Dict = None) -> Optional[StockScore]:
        ...
        # 一票否决项
        veto_reason = self._check_veto(stock_data)
        if veto_reason:
            return StockScore(
                strategy_type=self.strategy_type,
                stock_code=code,
        ...
        # 2. 首日换手率 (25分)
        turnover_score = self._score_new_stock_turnover(
            stock_data.get('first_day_turnover', 0),
            stock_data.get('is_small_cap', False)
        )
        ...
        return StockScore(
            ...
            signal_level=signal_level,
            suggested_position_pct=position_pct,
            buy_timing="上市当日尾盘(14:50-15:00)，禁止盘中买入",
            stop_loss_pct=-7.0,
            time_stop_days=3
        )
    
    def _check_veto(self, stock_data: Dict) -> str:
        """检查一票否决项"""
        ...
        if is_small_cap:
            if turnover > 90:
                return "小盘股首日换手率>90%，抛压过重"
            if turnover < 70:
                return "小盘股首日换手率<70%，惜售"
        else:
            if turnover > 80:
                return "首日换手率>80%，抛压过重"
            if turnover < 50:
                return "首日换手率<50%，惜售"
