...
class RSIOversoldAnalyzer:
    ...
    def find_oversold(self, df: pd.DataFrame, lookback: int = 60) -> List[Dict]:
        ...
        for i in range(self.rsi_period, len(df)):
            ...
            # 检查是否超卖
            if rsi < self.oversold_level and rsi > df.iloc[i-1]['rsi']:


                # 新增：过滤下跌趋势，20日线在60日线下放弃操作
                ma20 = df['close'].rolling(window=20).mean().iloc[i]
                ma60 = df['close'].rolling(window=60).mean().iloc[i]
                if ma20 < ma60:
                    continue
                # 检查之前是否处于正常或超买状态
    ...
    def analyze_stock(self, stock_code: str, stock_name: str = None) -> Optional[Dict]:
        ...
        try:
            ...
            # 全局趋势二次校验
            ma20_final = df['close'].rolling(20).mean().iloc[-1]
            ma60_final = df['close'].rolling(60).mean().iloc[-1]
            if ma20_final < ma60_final:
                return None
