...
def fetch_index_tencent() -> Dict[str, dict]:
    """获取上证/深证/创业板实时指数"""
    url = 'https://qt.gtimg.cn/q=sh000001,sz399001,sz399006,sh000300,sh000905,sh000852,sh000688,bj899050'
    headers = {'User-Agent': 'Mozilla/5.0', 'Referer': 'https://finance.qq.com'}
    ...
    for line in r.text.split('\n'):
        ...
        if not line:
            continue
        # sh000001 → 000001, sz399001 → 399001, sz399006 → 399006, bj899050 → 899050
        m = re.search(r'v_(sh\d+|sz\d+|bj\d+)="(.+)"', line)
        if not m:
            continue
        raw_code = m.group(1)
...
def get_index_realtime() -> Optional[dict]:
    ...
    return {
        ...
        'sz': data.get('399001', {}),
        'cy': data.get('399006', {}),
        'hs300': data.get('000300', {}),
        'zz500': data.get('000905', {}),
        'zz1000': data.get('000852', {}),
        'kc50': data.get('000688', {}),
        'bz50': data.get('899050', {})
    }
