...
class LimitUpAnalyzer:
    ...
    def _calc_sealing_strength(self, row: pd.Series) -> float:
        ...
        if open_count == 0:
            ...
            score -= 35
        print(f"炸板次数得分: {score}")
        # 3. 封单比评分（核心指标）

        seal_amount = self._safe_float(row.get('封板资金', 0))
        market_cap = self._safe_float(row.get('流通市值', 1))
        print(f"封板资金: {seal_amount}, 流通市值: {market_cap}, 封单比： {seal_amount / market_cap}%")
        if market_cap > 0:
        ...
        if open_count > 0 and last_t:
            lh, lm, _ = last_t
            last_min = lh * 60 + lm
            print("最后封板时间： ",lh, lm, last_min)
            # 14点前回封承接强，加分；尾盘回封只小幅补偿
            if last_min <= 14 * 60:
                score += 8
            else:
                score += 2
    ...
    def analyze_all_limit_up(self) -> List[Dict]:
       ...
       # print(f"{Fore.CYAN}   使用数据源: {self.fetcher.data_adapter.source}{Style.RESET_ALL}")
        ...
        for _, row in limit_up_df.iterrows():
            ...
            for idx, (_, row_self) in enumerate(stock_list.iterrows(), 1):
                self_stock_code = row_self['code']
                if self_stock_code == row['代码']:
                    
                
                    print("涨停：", row['代码'], "自选： ", self_stock_code)
                    # return
