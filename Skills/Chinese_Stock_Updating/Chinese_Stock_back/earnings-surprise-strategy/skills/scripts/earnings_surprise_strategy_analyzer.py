...
class EarningsSurpriseScanner:
    ...
    def save_stock_analysis_batch(self,
        analysis_list: List[Dict],
        save_dir: str = "/home/jarvis/.openclaw/workspace/skills/Chinese_Stock_back/stock_earnings_analysis/"  # 自动创建文件夹
    ) -> str:
        """
        批量保存股票分析 → 文件名带时间戳：2025-05-16_14-30-00.json
        永不覆盖！
        """
        # 创建目录
        ...
        filename = f"stock_earnings_analysis_{timestamp}.json"
        ...
        print(f"✅ 已保存【时间版】结果 → {save_path}")
        return save_path  # 返回路径，方便记录
    ...
    def scan_daily_earnings(self, date: str = None) -> List[Dict]:
        ...
        # self.save_stock_analysis_batch(all_stock_results)
        # 按得分排序
    ...
    def analyze_earnings(self, earnings: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            reasons += "财报超预期: " +surprise_analysis.get('level', '') + "; "
    ...
    def _analyze_institutional_attitude(self, stock_code: str) -> Dict:
        ...
        try:
            ...
            valid_days = 30  # 短线只看1个月内
            valid_cutoff = today - timedelta(days=valid_days)
            ...
            for r in ratings:
                ...
                if isinstance(r_date, datetime):
                    r_date = r_date.date()
                if r_date >= valid_cutoff:
                    recent_ratings.append(r)

                    # 收集有效目标价（>0）
                    tp = r.get('target_price', 0)
                    if tp and tp > 0:
                        valid_target_prices.append(tp)
            ...
            print("upgrades: ", upgrades, "downgrades: ", downgrades)
            # 目标价空间 to_do ..
