...
def calc_profit_target(code, cost, shares, trend_coef):
    ...
    # 向上取整到对应单位
    import math
    s1 = math.floor(raw_s1 / round_unit) * round_unit
    s2 = math.floor(raw_s2 / round_unit) * round_unit
    s3 = math.floor(raw_s3 / round_unit) * round_unit
    print("raw_s1", shares, cfg["sell_ratio"][0], cfg["sell_ratio"][1], cfg["sell_ratio"][2], raw_s1, raw_s2, raw_s3, ", res : ", s1, s2, s3)
    #  # 卖出股数
...
def run_analysis(session: str):
    ...
    for item in raw:
        h = Holding(
            ...
            real_cost=item.get('real_cost', item['cost']),
            shares=item['shares'],
...
def build_card(session: str, results: list, position_advice, total_asset, cash_info, date_str: str):
    ...
    for r in results:
        ...
        if r['tech']:
            ...
            if current_price >= pf['p1'] and not stop_profit_hit[0]:
                action_reasons.append(f"🎯 达到第一止盈{pf['p1']} / 当前价：{current_price} → 卖出{pf['s1']}股")
            if current_price >= pf['p2'] and not stop_profit_hit[1]:
                if not stop_profit_hit[0]:
                    action_reasons.append(f"🎯 达到第二止盈，且第一止盈未操作{pf['p2']} / 当前价：{current_price} → 卖出{pf['s2'] + pf['s1']}股") 
                else: 
                    action_reasons.append(f"🎯 达到第二止盈{pf['p2']} / 当前价：{current_price} → 卖出{pf['s2']}股")
            if current_price >= pf['p3'] and not stop_profit_hit[2]:
                if pf['s3'] == 0 and (r["shares"] == 100 or r['shares'] == 200):
                    action_reasons.append(f"🎯 达到第三止盈{pf['p3']} / 当前价：{current_price} → 卖出{r["shares"]}股 or 利益奔跑")
                else:
                    if not stop_profit_hit[0] and not stop_profit_hit[1]:
                        action_reasons.append(f"🎯 达到第三止盈，且第一\二止盈未操作{pf['p3']} / 当前价：{current_price} → 卖出{pf['s2'] + pf['s1'] + pf['p3']}股") 
                    elif stop_profit_hit[0] and not stop_profit_hit[1]: 
                        action_reasons.append(f"🎯 达到第三止盈，且第二止盈未操作{pf['p3']} / 当前价：{current_price} → 卖出{pf['s2']+ pf['p3']}股")
                    else:
    ...
    if need_action:
        ...
        for s in need_action:
            ...
            # 操作原因（第一条最重要）
            card_content_lines.append(f"{s['reasons'][-1]}")
            # 止盈止损
            pf = s['profit']
