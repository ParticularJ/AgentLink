       ...
       # print(result)
        ...
        for res in result:
            ...
            if res.get('score', 0) >= 70:     
                ...
                result_sig['TopSector'] = res.get('TopSector')
                if len(res.get('stock_code')) > 6:
                     result_sig['stock_code'] = res.get('stock_code')[2:]
...
def fuse_recommendations(recommendations: List[Dict], top_n: int = 5,
    ...
    for code, data in stock_map.items():
        ...
        if base_score < 70:
            print(f"股票 {data['stock_name']}({code}) 基础得分 {base_score:.1f} 低于70分，剔除推荐")
            continue
        ...
        # 早盘谨慎加分（只给真强势）
        # ------------------------------
        if session == 'MORNING' and final_best >= 82:
            morning_bonus = min(final_best - 80, 5)
            combined = min(combined + morning_bonus, 100)

        # 过滤低分
        if combined < 70:
            print(f"股票 {data['stock_name']}({code}) 综合得分 {combined:.1f} 低于70分，剔除推荐")
            continue
...
def build_report(top: List[Dict], session: str, total_recs: int) -> str:
    ...
    for i, s in enumerate(top, 1):
        ...
        if s["penalty"] < 0:
            lines.append(f'   新闻原因: {", ".join(s["penalty_reason"])}')
