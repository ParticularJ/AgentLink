...
RECO_DIR = '/home/jarvis/.openclaw/workspace/skills/Chinese_Stock/hot-spot-strategy/recommendations'
...
def build_card(date_str, session_label, recs, total_pos, success_count, no_result_count, err_count, filename):
    ...
    emojis = ["🔥", "✅", "📌", "📌", "📌"]
     # 展示板块信息
    top_sectors = recs[0].get('TopSectors') or [] if recs else []
    print(f"{Fore.CYAN}📊 当日涨停Top{len(top_sectors)}行业:{Style.RESET_ALL}")
    TopInfo = f"当日涨停Top{len(top_sectors)}行业:" + '\n'
    for idx, sector in enumerate(top_sectors, 1):
        ...
        TopInfo += f"{idx}. {sector['所属行业']} - {sector['涨停数']}只涨停 - 占比{sector['涨停占比']}% - 最高连板{sector['最高连板']}天 - {sector['强度']}" + '\n'
    print()
    ...
    card = {
        "header": {
            "title": {"tag": "plain_text", "content": f"{icon} 热点仓融合推荐 | {date_str} {session_label}"},
