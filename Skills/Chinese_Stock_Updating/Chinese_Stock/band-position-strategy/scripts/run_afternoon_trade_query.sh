...
card = {
    'header': {
        'title': {'tag': 'plain_text', 'content': '📊 盘中交易询问 | 15:00'},
        'subtitle': {'tag': 'plain_text', 'content': f'{datetime.now().strftime(\"%Y-%m-%d %H:%M\")} 尾盘时段'},
        'template': 'purple'
    },
    'elements': [
        ...
        {'tag': 'note', 'elements': [{'tag': 'plain_text', 'content': '⚠️ 尾盘时段，请注意交易时间'}]}
    ]
}
