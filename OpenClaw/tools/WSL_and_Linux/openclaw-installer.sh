...
show_help() {
    ...
    printf "    %s update       # 更新到最新版本\n" "$0"
    printf "    %s uninstall    # 完全卸载\n\n" "$0"
    printf "${GREEN}注意:${NC} 安装完成后请手动运行 'openclaw onboard' 进行配置\n"
...
check_nodejs() {
    ...
    print_msg "$GREEN" "✓ Node.js 版本: $new_version"
...
cmd_install() {
    ...
    print_msg "$BLUE" "[1/4] 环境准备..."
    sudo apt update && sudo apt install -y curl git build-essential

    print_msg "$BLUE" "[2/4] 检查 Node.js..."
    check_nodejs

    print_msg "$BLUE" "[3/4] 配置 npm..."
    setup_npm

    print_msg "$BLUE" "[4/4] 安装 OpenClaw..."
    ...
    # 自动执行交互式配置
    print_msg "$CYAN" "🔧 启动交互式配置向导..."
    printf "\n"
    if command_exists openclaw; then
        openclaw onboard
    else
        print_msg "$YELLOW" "⚠️  无法找到 openclaw 命令，请手动运行 'openclaw onboard' 进行配置"
    fi
}
...
cmd_update() {
    ...
    # 更新后重新运行配置向导（确保配置是最新的）
    printf "\n"
    print_msg "$CYAN" "🔧 是否重新运行配置向导? [Y/n]: "
    read -r run_onboard
